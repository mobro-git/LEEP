---
title: "Stacked Bar Presentation Figures"
author: "Ruying Gao"
date: "`r Sys.Date()`"
output: 
  html_document:
...
params:
  ...
  regions: c("United States", "Canada", "Mexico")
  mode: NULL       
---
...
tar_load(sb_presentation_figmap)
stopifnot(exists("sb_presentation_figmap"))
```
...
plot_list <- sb_presentation_figmap
```
...
df <- df %>% 
  filter(year %in% config$default_years) 
...
for (region in params$regions) {
  ...
  pdf(file=paste("output/round1/presentation/", str_replace(region, " ", "_"), "/stackbar_presentation.pdf",sep=""), width = 14, height = 6)
...
for (region in params$regions) {
  ...
  for (figure in unique(df$title_name)) {
    ...
    png(filename=paste("output/round1/presentation/", str_replace(region, " ", "_"), "/", 
                       str_replace_all(unique(dat$title_name), "\\|","_") , "_stacked_bar.png", sep = ""), width=1200, height=600)
