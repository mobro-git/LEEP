...
dot_plots_sens = function(plot_type, config, emf_data_long, figmap, figure_num, reg, ymin, ymax, plot_title, metric, historic_coord = c(0,0), iralow_coord = c(0,0), irahigh_coord = c(0,0)) {
  ...
  # preira_mean_30 = df_stat_30[df_stat_30$scenario == "Pre-IRA","mean"]$mean
  # ira_mean_30 = df_stat_30[df_stat_30$scenario == "IRA","mean"]$mean
  # preira_mean_35 = df_stat_35[df_stat_35$scenario == "Pre-IRA","mean"]$mean
  # ira_mean_35 = df_stat_35[df_stat_35$scenario == "IRA","mean"]$mean
  # preira_med_30 = df_stat_30[df_stat_30$scenario == "Pre-IRA","median"]$median
  # ira_med_30 = df_stat_30[df_stat_30$scenario == "IRA","median"]$median
  # preira_med_35 = df_stat_35[df_stat_35$scenario == "Pre-IRA","median"]$median
  ...
  #                    "Pre-IRA mean" = c(preira_mean_30, preira_mean_35, preira_med_30, preira_med_35),
...
```{r}
dot_plots = function(plot_type, config, emf_data_long, figmap, figure_num, reg, ymin, ymax, plot_title, metric, historic_coord = c(0,0), preira_coord = c(0,0), ira_coord = c(0,0)) {
  subpalettes = create_subpalettes(figmap_leep_timeseries, config)
  ...
  fig = fig + scale_y_continuous(limits = c(ymin, ymax)) +
      scale_alpha(range = c(0.6, 1), guide = "none") + 
      annotate("text", x = historic_coord[1], y = historic_coord[2], label = "Historic", color = "black", alpha = 1) +
      annotate("text", x = preira_coord[1], y = preira_coord[2], label = "Pre-IRA", color = "#0388B3", alpha = 1) +
...
lts_data_table = function(focus, leep_models) {
  ...
  dta_leep = clean_data %>% 
    filter(variable == focus, model %in% models_leep, year %in% c(2030, 2035), scenario != "Pre-IRA") %>%
    group_by(year, unit) %>%
    summarize(median = median(value), mean = mean(value)) %>%
    mutate(scenario = "IRA")
...
sens = dot_plots_sens("time_series", config, clean_data2, figmap_leep_timeseries, 16, "United States", 0, 6000, "Emissions", metric = "median", historic_coord = c(2015,5000), iralow_coord = c(2030, 4400), irahigh_coord = c(2027, 2200))
print(sens$full)
...
p = dot_plots("time_series", config, clean_data4, figmap_leep_timeseries, 18, "United States", 0, 100, "EV Share", metric = "mean", historic_coord = c(2015,8), preira_coord = c(2030, 16), ira_coord = c(2027, 45))
print(p$full)
print(p$stats)
...
fig = print_graph("stacked_bar", config, clean_data, figmap_leep_stackbar, 3, "United States", level_var = c("Lubricants", "Motorcycles", "Buses", "Pipelines", "Rail", "Water", "Aircraft", "MHD Trucks", "LD Trucks", "Passenger Cars")) +
  bottom1
...
emis_stack = function(dta, title) {
  p = dta %>% ggplot() +
    geom_bar(aes(x = year, y = value, fill = variable_rename), position = "stack", stat = "identity") +
