...
clean_data3 = rbind(raw, iea_2022) %>%
  filter(model != "EIA-LTS") %>% 
  duplicate_historical_data("IEA",2022) #%>% 
  # mutate(value = 1 * value)
  
# clean_data4 = clean_data %>% left_join(clean_data3, by = c("model", "scenario", "variable", "unit" , "datasrc", "year", "region"))

iea_2022 = clean_data %>% filter(model == "IEA", year == 2021) %>% mutate(year = 2022)
clean_data4 = rbind(clean_data, iea_2022) %>% filter(unit != "thousands") %>% mutate(value = 1000 * value) %>%
  filter(model != "EIA-LTS") %>% 
  duplicate_historical_data("IEA",2022)
...
p = dot_plots("time_series", config, clean_data4, figmap_leep_timeseries, 18, "United States", 0, 18, "EV Share", metric = "mean")
print(p$full)
print(p$stats)
...
totals = dta2 %>% group_by(model, scenario, region, datasrc, unit) %>%
  summarize(value = sum(value))

cap_dotplot = function(data, label, dot_color, var_list, ymin, ymax) {
  p = data %>% filter(variable %in% var_list) %>%
    group_by(model, scenario) %>% 
    summarize(value = sum(value)) %>% 
  ggplot() +
  geom_point(aes(x = 1, y = value, color = dot_color), shape = 1, size = 2) +
  scale_x_continuous(breaks = c(1), labels = c(label), limits = c(0.9, 1.1)) +
    scale_y_continuous(limits = c(ymin, ymax)) +
    theme_emf() +
    scale_subpalette(subpalettes, "Emissions|CO2|Energy|Demand|Industry") +
    theme(panel.grid = element_blank(), plot.title = element_blank(),
...
dot_total = totals %>% ggplot() +
  geom_point(aes(x = 1, y = value, color = "black"), shape = 1, size = 2) +
  scale_x_continuous(breaks = c(1), labels = c("Total"), limits = c(0.9, 1.1)) +
    scale_y_continuous(limits = c(ymin, ymax)) +
    theme_emf() +
    scale_subpalette(subpalettes, "Emissions|CO2|Energy|Demand|Industry") +
    theme(panel.grid = element_blank(), plot.title = element_blank(),
...
dot_solar = cap_dotplot(dta2, label = "Solar", dot_color = "yellow", 
                        var_list = c("Capacity Additions|Electricity|Solar", "Capacity Retirements|Electricity|Solar"),
                        ymin = ymin, ymax = ymax)
dot_wind = cap_dotplot(dta2, label = "Wind", dot_color = "yellow", 
                        var_list = c("Capacity Additions|Electricity|Wind", "Capacity Retirements|Electricity|Wind"),
                        ymin = ymin, ymax = ymax)
dot_coal = cap_dotplot(dta2, label = "Coal", dot_color = "yellow", 
                        var_list = c("Capacity Additions|Electricity|Coal|w/o CCS", "Capacity Retirements|Electricity|Coal|w/o CCS"),
                        ymin = ymin, ymax = ymax)
dot_gas = cap_dotplot(dta2, label = "Gas", dot_color = "yellow", 
                        var_list = c("Capacity Additions|Electricity|Gas|w/o CCS", "Capacity Retirements|Electricity|Gas|w/o CCS"),
                        ymin = ymin, ymax = ymax)
dot_nuclear = cap_dotplot(dta2, label = "Nuclear", dot_color = "yellow", 
                        var_list = c("Capacity Additions|Electricity|Nuclear", "Capacity Retirements|Electricity|Nuclear"),
                        ymin = ymin, ymax = ymax)
dot_hydro = cap_dotplot(dta2, label = "Hydro", dot_color = "yellow", 
                        var_list = c("Capacity Additions|Electricity|Hydro", "Capacity Retirements|Electricity|Hydro"),
                        ymin = ymin, ymax = ymax)
dot_storage = cap_dotplot(dta2, label = "Storage", dot_color = "yellow", 
                        var_list = c("Capacity Additions|Electricity|Storage Capacity", "Capacity Retirements|Electricity|Storage Capacity"),
                        ymin = ymin, ymax = ymax)
dot_coalccs = cap_dotplot(dta2, label = "Coal w/ CCS", dot_color = "yellow", 
                        var_list = c("Capacity Additions|Electricity|Coal|w/ CCS", "Capacity Retirements|Electricity|Coal|w/ CCS"),
                        ymin = ymin, ymax = ymax)
dot_gasccs = cap_dotplot(dta2, label = "Gas w/ CCS", dot_color = "yellow", 
                        var_list = c("Capacity Additions|Electricity|Gas|w/ CCS", "Capacity Retirements|Electricity|Gas|w/ CCS"),
                        ymin = ymin, ymax = ymax)
