...
```{r}
lts_coneplot_with_dots = function(config, data, figmap, lts_fignum, leep_fignum, region, ymin, ymax, title, metric = "mean", facet = FALSE) {
...
# Industrial Energy
p4 = lts_coneplot_with_dots(config, clean_data, figmap_leep_cone, 13, 14, "United States", 0, 25, title = "Industrial Sector Final Energy", "median", facet = TRUE) +
...
lts_data_table = function(focus, leep_models) {
  ...
  dta_leep = clean_data10 %>% 
    filter(variable == focus, model %in% models_leep, year %in% c(2030, 2035), scenario == "IRA") %>%
    group_by(year, unit) %>%
    summarize(median = median(value)) %>%
    mutate(scenario = "IRA")
...
# Trying again: cones
# Buildings Energy
mid = lts_coneplot_with_dots(config, clean_data, figmap_leep_cone, 23, 24, "United States", -100, 2500, title = "Buildings", "median", facet = TRUE) +
...
# Transportation Energy
left = lts_coneplot_with_dots(config, clean_data, figmap_leep_cone, 21, 22, "United States", -100, 2500, title = "Transportation" , "median", facet = TRUE)
left
# p3 + facet_grid(~factor(variable_rename, levels=c('Fossil', 'Electricity', 'Alternative Fuels')))
# ggsave(filename = "output/leep/cone_uncertainty/Fig6.3_LTS_cone_transport_facet.svg",p3, width = 7, height = 5, units = "in")

# Industrial Energy
right = lts_coneplot_with_dots(config, clean_data, figmap_leep_cone, 25, 26, "United States", -100, 2500, title = "Industrial", "median", facet = TRUE) +
  theme(axis.title.y = element_blank(), axis.text.y = element_blank())
right
# +
#   facet_grid(~factor(variable_rename, levels=c('Fossil', 'Electricity', 'Alternative Energy')))
...
# Economy-wide Emissions
top = lts_coneplot_with_dots(config, clean_data, figmap_leep_cone, 19, 20, "United States", -100, 2500, title = "Electric Sector CO2 Emissions", "median", facet = FALSE)
top
# +
#   facet_grid(~factor(variable_rename, levels=c('Electricity', 'Transportation', 'Buildings', 'Industry')))
# p5
...
combined = (top / (left | mid | right))
combined

ggsave(filename = "output/leep/cone_uncertainty/Fig6.2NEW_LTS_cone_indirect_emissions_TOTAL.svg", combined, width = 7, height = 5, units = "in")
...
dta = data_from_graph("time_series", config, clean_data7, figmap_leep_timeseries, 122, "United States")
# View(dta)
```
...
p1 = emis_stack(dta1, "Economy-Wide CO2 Emissions")
p1
...
sens_dot_plot = function(dta, title, far_left = FALSE, ymin = 0, ymax = 0, ira_coord = c(0,0), low_coord = c(0,0), high_coord = c(0,0)) {
  if (far_left) {
  ...
  if (far_left) {
    ...
    segment_code = geom_segment(data = stats, 
                                aes(x = year + stagger - 0.5, xend = year + stagger + 0.5, 
  ...
  if (!far_left) {
    p = p + 
      geom_text_repel(aes(x = year, y = value + 50, label = scenario), size = 2, hjust = -0.2) +
  ...
  if(sum(low_coord) != 0) {
    p = p +
      annotate("text", x = low_coord[1], y = low_coord[2], size = 2.5,
               label = "Pessimistic", color = "#42d4f4", alpha = 1, hjust = -0.2)
  }
  if(sum(high_coord) != 0) {
    p = p +
      annotate("text", x = high_coord[1], y = high_coord[2], size = 2.5,
               label = "Optimistic", color = "#883192", alpha = 1, hjust = -0.2)
  }
...
p1 = sens_dot_plot(sens_dta_panel1, title = "IRA Implementation", ymin = 0, ymax = 1600, far_left = TRUE, ira_coord = c(2030,1275), low_coord = c(2030, 1425), 
                   high_coord = c(2030, 250))
# p1
...
ggsave(filename = "output/leep/dots/Fig1.4b_elec_sensitivity_dots.svg",combined, width = 7, height = 5, units = "in")

sens_dta = clean_data %>% filter(variable == "Emissions|CO2", year %in% c(2030, 2035))
...
sens_dta_econ = clean_data %>% filter(variable == "Emissions|CO2",
                                 year %in% c(2030, 2035)) %>%
...
p7 = sens_dot_plot(sens_dta_econ, title = "Economy-Wide CO2 Emissions", ymin = 0, ymax = 5000, far_left = TRUE, ira_coord = c(2030,4000), low_coord = c(2029.2, 4200), 
                   high_coord = c(2030.6, 3700))
p7
ggsave(filename = "output/leep/dots/Fig1.4a_econwide_sensitivity_dots.svg",p7, width = 7, height = 5, units = "in")


p8 = sens_dot_plot(sens_dta_trans, title = "Transportation CO2 Emissions", ymin = 0, ymax = 2000, far_left = TRUE, ira_coord = c(2030,1700), low_coord = c(2028.3, 1600), 
                   high_coord = c(2030.6, 1550))
p8
ggsave(filename = "output/leep/dots/Fig1.4c_trans_sensitivity_dots.svg",p8, width = 7, height = 5, units = "in")


p9 = sens_dot_plot(sens_dta_build, title = "Buildings CO2 Emissions", ymin = 0, ymax = 750, far_left = TRUE, ira_coord = c(2030,600), low_coord = c(2029.3, 625), 
                   high_coord = c(2030.7, 550))
p9
ggsave(filename = "output/leep/dots/Fig1.4d_build_sensitivity_dots.svg",p9, width = 7, height = 5, units = "in")


p10 = sens_dot_plot(sens_dta_ind, title = "Industrial CO2 Emissions", ymin = 0, ymax = 1250, far_left = TRUE, ira_coord = c(2030,1125), low_coord = c(2028.3, 1050), 
                   high_coord = c(2030.7, 1080))
p10
ggsave(filename = "output/leep/dots/Fig1.4e_ind_sensitivity_dots.svg",p10, width = 7, height = 5, units = "in")
