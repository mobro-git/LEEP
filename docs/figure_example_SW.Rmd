...
duplicate_historical_data <- function(data_long, hist_model, hist_year) {
  ...
  data_wide = data_in %>% left_join(select(copy_data,c(value,year,variable,region)), by = c("year", "variable","region")) %>%
    mutate(value = case_when(
      year == 2020 & !is.na(value.y) ~ value.y,
      TRUE ~ value.x
    )) %>%
    select(-c(value.x,value.y))
  
  data_final = rbind(data_wide, hist_data, data_cdr)                        
...
lts_coneplot_with_dots = function(config, data, figmap, lts_fignum, leep_fignum, region, ymin, ymax, metric = "mean", facet = FALSE) {
  ...
  scale_y_continuous(limits = c(ymin, ymax))# + ggtitle("LTS")
  
  # extract points and summary statistics for 2030 and 2035
  df_30 = data_from_graph("cone_uncertainty", config, data2, figmap, leep_fignum, region) %>%
    filter(year %in% c(2030))
  df_stat_30 = df_30 %>%
    group_by(scenario,year,variable_rename) %>%
    summarise(mean = mean(value),
  ...
  df_35 = data_from_graph("cone_uncertainty", config, data2, figmap, leep_fignum, region) %>%
    filter(year %in% c(2035))
  df_stat_35 = df_35 %>%
    group_by(scenario,year,variable_rename) %>%
    summarise(mean = mean(value),
...
```{r}
# fig3 = print_graph("cone_uncertainty", config, clean_data, figmap_leep_cone, 3, "United States") +
#   scale_y_continuous(limits = c(-1000, 2500)) + ggtitle("LTS")
# fig3
# df3 = data_from_graph("cone_uncertainty", config, clean_data2, figmap_leep_cone, 3, "United States")
# 
# fig4 = print_graph("cone_uncertainty", config, clean_data2, figmap_leep_cone, 4, "United States") +
#   scale_y_continuous(limits = c(-1000, 2500)) + ggtitle("IRA") +
#   theme(axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank())
# fig4
# df4 = data_from_graph("cone_uncertainty", config, clean_data2, figmap_leep_cone, 4, "United States")
# 
# final2 = fig3 + fig4 +
#     plot_annotation(title = "Energy Emissions", theme = theme(plot.title = element_text(hjust = 0.5))) +
...
```{r}
# fig5 = print_graph("cone_uncertainty", config, clean_data, figmap_leep_cone, 5, "United States") +
#   scale_y_continuous(limits = c(0, 30)) + ggtitle("LTS")
# fig5
# 
# fig6 = print_graph("cone_uncertainty", config, clean_data2, figmap_leep_cone, 6, "United States") +
#   scale_y_continuous(limits = c(0, 30)) + ggtitle("IRA") +
#   theme(axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank())
# fig6
# 
# final3 = fig5 + fig6 +
#     plot_annotation(title = "Transportation Energy", theme = theme(plot.title = element_text(hjust = 0.5))) +
#     plot_layout(guides = "collect", widths = c(1,1)) +
#     theme(plot.margin = margin(0,0,0,0))
# final3
...
```{r}
# fig7 = print_graph("cone_uncertainty", config, clean_data, figmap_leep_cone, 7, "United States") +
#   scale_y_continuous(limits = c(0, 25)) + ggtitle("LTS")
# fig7
# 
# fig8 = print_graph("cone_uncertainty", config, clean_data2, figmap_leep_cone, 9, "United States") +
#   scale_y_continuous(limits = c(0, 25)) + ggtitle("IRA") +
#   theme(axis.title.y = element_blank(), axis.text.y = element_blank(), axis.ticks.y = element_blank())
# fig8
# 
# final4 = fig7 + fig8 +
#     plot_annotation(title = "Industrial Energy", theme = theme(plot.title = element_text(hjust = 0.5))) +
#     plot_layout(guides = "collect", widths = c(1,1)) +
#     theme(plot.margin = margin(0,0,0,0))
# final4
