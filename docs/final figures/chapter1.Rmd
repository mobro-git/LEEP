...
EconDot
EconSpg$data = clean_supplemental_data(EconSpg$data, paste(fig_no, "a"))
saveall(EconDot, EconSpg$data, paste(fig_no, "a"))
...
### Energy-only CO2 range comparison

```{r}
econ_noprocess_data = spg_clean(
  123456,
  c("IPM-NRDC", "IPM-EPA", "ReEDS-NREL", "Scout-LEEP", "Haiku-RFF"),
  "EPA-GHGI",
  config,
...
econ_noprocess_tab = html(econ_noprocess_data, "Economy-Wide CO2 - No Processes")
econ_noprocess_tab
```
...
all_panels %>% write.csv("./output/final_figures/data/Fig1.5_Summary.csv", row.names = FALSE)
# 
# 
