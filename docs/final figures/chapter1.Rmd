...
df = data_from_graph("stacked_bar", config, clean_data, figmap_leep_stackbar, 9, "United States") %>%
  mutate(variable_rename = factor(variable_rename, 
                                  levels = c("Industry: Process","Industry: Indirect","Industry: Direct",
                                             "Buildings: Indirect","Buildings: Direct", 
                                              "Transportation: Indirect","Transportation: Direct")))
...
EconDot
hold = EconSpg$data
EconSpg$data = clean_supplemental_data(EconSpg$data, paste0(fig_no, "_main"))
saveall(EconDot, data.frame(), paste0(fig_no,"_main"))

delta = delta(25, c("IPM-NRDC", "IPM-EPA", "ReEDS-NREL", "Scout-LEEP", "Haiku-RFF"), config, clean_data, figmap_leep_timeseries)
delta

delta$df$variable_rename = "% Difference from No IRA"
delta$df$unit = "%"
delta$df$variable = "Emissions|CO2|% Difference from No IRA"
delta$df = clean_supplemental_data(delta$df, paste0(fig_no,"_delta"))
saveall(delta$figure, data.frame(), paste0(fig_no,"_delta"), wd = 3.012, ht = 3)

df_clean = bind_rows(EconSpg$data, delta$df)
savedata(df_clean,fig_no)
EconSpg$data = hold
...
# ----------------------------- ranges with a subset of models used for the BR Voluntary Supplement
brvs = indicators_all %>% 
  filter(model %in% c("EPS-EI","GCAM-CGS","GCAM-PNNL","MARKAL-NETL","NEMS-OP","NEMS-RHG","REGEN-EPRI","RIO-REPEAT","USREP-ReEDS")) %>%
  filter(scenario == "IRA")
brvs_indicators = brvs %>% left_join(
...
data_combo = bind_rows(Elec_data, Tran_data, Buil_data, Ind_data, EconSpg$data)
savedata(data_combo, paste0(fig_no,"_alldata"))
saveall(combined, summary, fig_no)
#savedata(summary, fig_no)
...
data_combo = bind_rows(CoalSpg$data, GasSpg$data, PetrolSpg$data)
...
p7 = sens_dot_plot(sens_dta_econ, title = "\nIRA Implementation", figmap_leep_timeseries, config, 
                   ymin = ymin_val, ymax = 5000, far_left = TRUE, single = FALSE, ps = FALSE,
                   ira_coord = c(2031.2,2600), low_coord = c(2031.3, 3760),
                   high_coord = c(2030.4, 4070))
p7$plot = p7$plot +
...
p7$plot = p7$plot +
  ...
  annotate("text", x = 2028.4, y = 4995, size = 2,
               label = "EPS-EI", color = "#A1A1A1", alpha = 1, hjust = 0) +
  annotate("text", x = 2028.4, y = 4845, size = 2,
             label = "GCAM-CGS", color = "#A1A1A1", alpha = 1, hjust = 0) +
  annotate("text", x = 2028.4, y = 4695, size = 2,
             label = "REGEN-EPRI", color = "#A1A1A1", alpha = 1, hjust = 0) +
  annotate("text", x = 2028.4, y = 4545, size = 2,
             label = "RIO-REPEAT", color = "#A1A1A1", alpha = 1, hjust = 0) +
  annotate("text", x = 2028.4, y = 4395, size = 2,
...
p7d$plot = p7d$plot +
  scale_shape_manual(breaks = c(-0.6, 0.6), values = c(1,0)) +
  annotate("text", x = 2028.4, y = 4995, size = 2,
...
econ_panel4_summ = econ_panel4_summ %>%
  group_by(variable, unit, year) %>%
  summarize(opt_median = median(diff_opt, na.rm = TRUE),
            opt_min = min(diff_opt, na.rm = TRUE),
            opt_max = max(diff_opt, na.rm = TRUE),
            pess_median = median(diff_pess, na.rm = TRUE),
            pess_min = min(diff_pess, na.rm = TRUE),
            pess_max = max(diff_pess, na.rm = TRUE)) %>%
  pivot_longer(cols = starts_with(c("opt_", "pess_")), names_to = c("scen","metric"), names_sep = "_") %>%
  #mutate(value = round(value,1)) %>%
  pivot_wider(names_from = scen, values_from = value) %>%
...
all_panels %>% write.csv("./output/final_figures/data/Fig1.5_Display.csv", row.names = FALSE)
# 
# 
...
### Alternates: economy-wide co2 emissions sensitivities

```{r}
fig_no = "1.5.ew_sensitivities_COLOR"
#fig_no = "1.5.ew_sensitivities_BW"
...
p7 = sens_dot_plot(sens_dta_econ, title = "\nIRA Implementation", figmap_leep_timeseries, config, 
                   ymin = ymin_val, ymax = 4100, far_left = TRUE, single = FALSE, ps = FALSE,
                   ira_coord = c(2031.2,2600), low_coord = c(2031.3, 3760),
                   high_coord = c(2030.4, 4070))
p7$plot = p7$plot +
  theme(axis.ticks = element_line(color = "black"),
...
p7d$plot = p7d$plot +
  scale_shape_manual(breaks = c(-0.6, 0.6), values = c(1,0)) +
  scale_color_manual(breaks = c("All-Adv", "Mod", "Constr"), 
                     values = c("#f15a22", "black", "#876123"))
  # annotate("text", x = 2028.4, y = 4995, size = 2,
  #          label = "GCAM-PNNL (L)", color = "#A1A1A1", alpha = 1, hjust = 0) + 
  # annotate("text", x = 2028.4, y = 4845, size = 2,
  #              label = "USREP-ReEDS (R)", color = "#A1A1A1", alpha = 1, hjust = 0) #FFF362
...
SENSITIVITIES = FALSE
if (SENSITIVITIES) {
  fig_no = "1.3X.summary_ranges_sens"
} else {
  fig_no = "1.3X.summary_ranges_main"
}
...
if (SENSITIVITIES) {
  indicators_all_ira = indicators_all %>% filter(!(grepl("No IRA", scenario)))
} else {
  indicators_all_ira = indicators_all %>% filter(scenario == "IRA")
}
indicators_all_noira = indicators_all %>% filter(scenario == "No IRA")
...
indicators_all_ira = indicators_all_ira %>% 
  left_join(
    (indicators_all_noira %>% mutate(value_noira = value) %>% select(model, variable, year, value_noira)),
    by = c("model", "variable", "year")) %>%
  mutate(pct_diff = -100 * (value - value_noira) / value_noira) %>%
  left_join(variable_rename, by = "variable") %>%
    select(-variable)

data_extract = indicators_all_ira %>% 
  rename("variable" = "sector") %>% 
  clean_supplemental_data(fig_no) %>% 
  mutate(pct_diff = indicators_all_ira$pct_diff)

indicators_summ = indicators_all_ira %>% 
  mutate(scenario = "IRA") %>%
  mutate(scenario = factor(scenario, levels = c("IRA"))) %>%
  mutate(category = interaction(sector, scenario)) %>%
  mutate(category = factor(category, levels = c(
      "Industry.IRA",
      "Buildings.IRA",
...
indicators_all_ira = indicators_all_ira %>% mutate(scenario = "IRA")

p1 = indicator_smallX(indicators_all_ira, indicators_summ, "Economy-Wide", bottom = TRUE, top = FALSE,
                     subtitle = expression(paste("Economy-Wide")))
p2 = indicator_smallX(indicators_all_ira, indicators_summ, "Electricity", bottom = FALSE, top = TRUE,
                     subtitle = expression(paste("Electricity")))
p3 = indicator_smallX(indicators_all_ira, indicators_summ, "Transportation", 
                     subtitle = expression(paste("Transportation (Direct + Indirect from Electricity)")))
p4 = indicator_smallX(indicators_all_ira, indicators_summ, "Buildings", 
                     subtitle = expression(paste("Buildings (Direct + Indirect from Electricity)")))
p5 = indicator_smallX(indicators_all_ira, indicators_summ, "Industry", bottom = FALSE, top = FALSE,
                     subtitle = expression(paste("Industry (Direct + Indirect from Electricity)")))
...
saveall(combined, data_extract, fig_no)
```
