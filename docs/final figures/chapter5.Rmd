...
p = pie1 %>% mutate(value = (100 * value) / total) %>% arrange(desc(value)) %>% ggplot() +
  ...
  geom_text(aes(x = "2021", y = value, label = paste0(Industry,": ", round(value,1), "%")), 
                position = position_stack(vjust = 0.5), stat = "identity", size = 3) +
  ...
  theme_emf() + 
  theme(axis.ticks.x = element_blank(), axis.ticks.y = element_line(color = "black"), axis.ticks.length = unit(-0.15, "cm"),  legend.position = "none",
        axis.title.x = element_blank())
p

saveall(p, pie1, fig_no, wd=5, ht=5)
```
