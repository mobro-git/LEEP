...
fig = print_graph("stacked_bar", config, clean_data, figmap_leep_stackbar, 3, "United States", 
  ...
  scale_fill_manual(labels = c("Other", "Pipelines", "Water", "Aircraft", "MHD Trucks", "LD Trucks", "Passenger Cars"),
                    values = c(sp[['Other']], sp[['Pipelines']], sp[['Water']], sp[['Aircraft']], 
                               sp[['MHD Trucks']], sp[['LD Trucks']], sp[['Passenger Cars']])) +
  labs(y = expression("Emissions (Mt CO2/yr)")) +
...
# TODO: OP-NEMS should only have reporting every 5 years
...
output = spg2(df = clean_data_evshare, title = "", yname = expression("EV Sales Share (%)"), gd = "none", ymin = 0, ymax = 100, 
...
A = ggplot(data = data, aes(x = axe, y = value,  color = scenario)) +
  ...
  annotate("text", x = 1.77, y = 325, label = "No IRA", color = "#F28063", alpha = 1) +
  annotate("text", x = 2.17, y = 375, label = "IRA", color = "#0388B3", alpha = 1) +
  ylim(0, 700)
