...
fig = dotted(TranSpg$data, TranSpg, "median", 0, 2500)
fig
...
# TODO: OP-NEMS should be showing up here, but its removed when year colums are dropped in raw
...
clean_data4 = rbind(raw, atr_2022, nems) %>% filter(unit != "thousands") %>% mutate(value = 100 * value) %>%
...
p = dot_plots("time_series", config, clean_data4, figmap_leep_timeseries, 118, "United States", 0, 100, "EV Share", metric = "median", hist_year = 2022, historic_coord = c(2015,8), preira_coord = c(2030, 10), ira_coord = c(2027, 53))
p$full
