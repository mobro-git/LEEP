...
sens_dta_panel4 = sens_dta %>% filter(scenario %in% panel4_scens) %>% 
  ...
  #                             TRUE ~ scenario)) %>%
  mutate(stagger = case_when(model == "GCAM-PNNL" ~ -0.7,
                             model == "ReEDS-NREL" ~ 0,
                             model == "USREP-ReEDS" ~ +0.7,
                             TRUE ~ 0.0))
  bind_rows(filler)
...
p1$plot = p1$plot + 
  scale_shape_manual(breaks = c(-0.7, 0, 0.7), values = c(0,1,5)) +
...
p3$plot = p3$plot + 
  scale_shape_manual(breaks = c(-0.4, 0.4), values = c(1,5)) +
  annotate("text", x = 2028.4, y = 1595, size = 3,
