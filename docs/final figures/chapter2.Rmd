...
sens_dta_panel1 = sens_dta %>% filter(scenario %in% panel1_scens) %>%
  ...
  mutate(stagger = case_when(
    scenario == "IRA.Low" ~ -0.8,
    scenario == "IRA.High" ~ 0.8,
    TRUE ~ 0
  ))
...
sens_dta_panel4 = sens_dta %>% filter(scenario %in% panel4_scens) %>% 
  ...
  #                             TRUE ~ scenario)) %>%
  mutate(stagger = case_when(model == "GCAM-PNNL" ~ -0.8,
                             model == "ReEDS-NREL" ~ 0,
                             model == "USREP-ReEDS" ~ +0.8,
                             TRUE ~ 0.0))
  bind_rows(filler)
...
p1 = sens_dot_plot(sens_dta_panel1, title = "\nIRA Implementation", 
                   ...
                   ira_coord = c(2030.0,965), low_coord = c(2028.7, 1190), 
...
p1$plot = p1$plot + 
  scale_shape_manual(breaks = c(-0.8, 0, 0.8), values = c(0,1,5)) +
...
p2$plot = p2$plot + 
  scale_shape_manual(breaks = c(0), values = c(3)) +
  annotate("text", x = 2028.4, y = 1595, size = 2,
               label = "GCAM-PNNL", color = "#A1A1A1", alpha = 1, hjust = 0)

p3$plot = p3$plot + 
  scale_shape_manual(breaks = c(-0.6, 0.6), values = c(4,3)) +
  annotate("text", x = 2028.4, y = 1595, size = 2,
...
p4$plot = p4$plot + 
  scale_shape_manual(breaks = c(-0.8, 0, 0.8), values = c(8,4,3)) +
  annotate("text", x = 2028.4, y = 1595, size = 2,
               label = "GCAM-PNNL (L)", color = "#A1A1A1", alpha = 1, hjust = 0) +
  annotate("text", x = 2028.4, y = 1545, size = 2,
