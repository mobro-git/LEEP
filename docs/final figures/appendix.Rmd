...
figure = ggplot() + 
  geom_line(data = hist_clean, aes(x = year, y = value), color = "black", size = 0.6) + 
  geom_line(data = mods_clean, aes(x = year, y = value, color = model), size = 0.6) + 
  geom_line(data = proj_clean, aes(x = year, y = value, group = model,  fill = "gray"), alpha = 0.2) + 
  ...
  labs(
    y = expression(paste("Natural Gas Prices (2020$/MMBtu)")), 
    x = "", 
    title = ""
  ) + 
  theme_classic() + 
  annotate("text", x = 2010, y = 11.5, label = "Historical", color = "black") + 
  annotate("text", x = 2020, y = 10, label = "AEO reference", color = "gray") + 
  annotate("text", x = 2020, y = 9.3, label = "projections (2015-2020)", color = "gray") + 
      theme(axis.text.x = element_text(angle = 45, hjust = 1),
          axis.ticks = element_blank())
...
p = dta_top %>% ggplot() +
  ...
  scale_linetype_manual(values = c("longdash","solid","dotted"), breaks = c("Advanced", "Moderate", "Conservative"), guide = T) +
