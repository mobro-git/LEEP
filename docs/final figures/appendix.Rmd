...
# Appendix A: Summary of annyal emission reductions
...
```{r}
fig_no = "D.1.naturalgasprice"

subpalettes = create_subpalettes(figmap_leep_timeseries, config)

pipe_proj = clean_data %>%
filter(variable == "Price|Primary Energy|Gas") %>%
  filter(model %in% c("GCAM-PNNL", "NEMS-EIA", "NEMS-OP", "USREP-ReEDS")) %>%
  filter(year %in% c(2025, 2030, 2035)) %>%
  mutate(value = value * 1.04) %>% #USD2018 to USD2020 via BLS Inflation Calculator 
  mutate(value = value / 0.947817) %>% #GJ to MMBtu
  mutate(unit = "2020$/MMBtu") %>%
  filter(scenario == "No IRA") %>%
  select(unit, model, year, value)
  

hist = readxl::read_xlsx("data-extra/ira_comparison_raw/input_assumptions.xlsx", sheet = "natural gas price", range = "K1:AJ2") %>%
  mutate(unit = "2020$/MMBtu") %>%
  pivot_longer(cols = !unit, names_to = "year") %>%
...
hist_clean = hist %>%
  filter(year >= 2005) %>%
  filter(year <= 2035) %>%
  select(model,year,value,unit)

proj = readxl::read_xlsx("data-extra/ira_comparison_raw/input_assumptions.xlsx", sheet = "natural gas price", range = "A1:J29") %>%
  pivot_longer(cols = c("2015", "2020", "2025", "2030", "2035", "2040", "2045", "2050"), names_to = "year") %>%
  filter(model != "Historic") %>%
  mutate(year = as.numeric(year))

proj = rbind(pipe_proj, proj)

mods_clean = proj %>%
  filter(!grepl("aeo", model)) %>%
  filter(!grepl("eea", model))%>%
  filter(year <= 2035) %>%
  select(model,year,value,unit)

mods = unique(mods_clean$model)
...
figure = ggplot() +
  ...
  scale_x_continuous(breaks = c(2005, 2010, 2015, 2020, 2025, 2030, 2035)) +
  scale_subpalette(subpalettes, "Emissions|CO2|Percent difference from No IRA") +
  labs(y = expression(paste("Natural Gas Prices (2020$/MMBtu)")),
       x = "",
       title = "") +
  theme_classic() +
  annotate(
...
df = rbind(hist_clean,proj_clean,mods_clean) %>%
  filter(!is.na(value)) %>%
  mutate(figure_num = fig_no,
         scenario = "Historic",
         label = "Natural Gas Price")
  
saveall(figure, df, fig_no)
```

## Capital Costs

```{r} 
fig_no = "D.2.capitalcosts_bat_ngcc_sol_wnd"
...
subpalettes$`Sox pct diff`[["OP-NEMS-Mod"]] = "blue"
subpalettes$`Sox pct diff`[["OP-NEMS-Adv"]] = "blue"

tech_renamer = c(
  "Utility-Scale Solar PV" = "Solar PV",
  "Wind Onshore" = "Onshore Wind",
  "NGCC with CCS" = "New NGCC CCS",
  "NGCC" = "NGCC",
  "Battery Storage (4-Hour)" = "Battery (4-Hour)",
  "Coal CCS Retrofit" = "Coal CCS Retrofits"
)
...
dta = dta %>%
  ...
  mutate(version = case_when(
    ...
    model == "OP-NEMS-Mod" ~ "Moderate",
    model == "OP-NEMS-Adv" ~ "Advanced",
    TRUE ~ "Moderate"
  )) %>%
...
p = dta_top %>% ggplot() +
  ...
  scale_linetype_manual(values = c("longdash","solid","dotted"), breaks = c("Advanced", "Moderate", "Conservative")) +
  facet_grid(. ~ variable, labeller = labeller(variable = tech_renamer)) +
  theme_emf() +
  theme(
          axis.ticks = element_line(color = "black"),
          axis.ticks.length = unit(-0.15, "cm"), axis.text.x = element_text(angle = 45, hjust = 1),
        axis.title.x = element_blank(), strip.text.x = element_text(face = "plain", size = 8),
        legend.text = element_text(size = 6), legend.key.size = unit(15,"point"))
...
saveall(p,data.frame(),fig_no)
dta_top = dta_top %>% mutate(figure_num = fig_no, label = variable, variable = "Capital Cost")
fig_no = "D.3.capitalcosts_coalgasCCS"
saveall(p2,data.frame(),fig_no)
dta_bottom = dta_bottom %>% mutate(figure_num = fig_no, label = variable, variable = "Capital Cost")
data_combo = bind_rows(dta_top, dta_bottom)
savedata(data_combo, "FigD.2+3.capitalcosts")
```
...
data_combo = combine_4c_data(fig$ts_df, fig$pd_df, fig$ad_df, metric, unit)
write.csv(data_combo, file = paste0("output/final_figures/data/Fig",fig_no,".csv"), row.names = FALSE)
...
fig = four_corners("Power sector CO2 Emissions",ts_map_ID, pd_map_ID, ad_map_ID, drop, histsrc, metric, unit, fig_no, ymin, ymax, brk)
...
fig = four_corners("Primary Energy Coal",ts_map_ID, pd_map_ID, ad_map_ID, drop, histsrc, metric, unit, fig_no, ymin, ymax, brk)
...
fig = four_corners("Primary Energy Gas",ts_map_ID, pd_map_ID, ad_map_ID, drop, histsrc, metric, unit, fig_no, ymin, ymax, brk)
...
fig = four_corners("Primary Energy Petroleum", ts_map_ID, pd_map_ID, ad_map_ID, drop, histsrc, metric, unit, fig_no, ymin, ymax, brk)
...
```{r}
fig_no = "E.12.CoalCCSGen"

#Only includes Coal CSS Generation
...
#Absolute Difference
adfigure = ad(diff_ID = ad_map_ID, "", "Generation", "none", drop)
...
fig = (NoIRAfigure |
         IRAfigure) / (adfigure$figure | plot_spacer()) +
  plot_layout(guides = "collect") 

fig

ts_df$figure_num = fig_no
adfigure$ad_df$figure_num = fig_no
data_combo = combine_4c_data(ts_df, data.frame(), adfigure$ad_df, metric, unit)
write.csv(data_combo, file = paste0("output/final_figures/data/Fig",fig_no,".csv"), row.names = FALSE)
...
```{r}
fig_no = "E.13.GasCCSGen"

#Only includes Gas CSS Generation
...
#          IRAfigure) / (adfigure$figure | pdfigure$figure) +
#   plot_layout(guides = "collect") +
#   plot_annotation(title = "Electricity Generation from Gas with CCS")
...
```{r}
fig_no = "E.14.CoalnoCCSGen" # w/o CCS
...
fig = four_corners("Electricity Generation from Coal without CCS", ts_map_ID, pd_map_ID, ad_map_ID, drop, histsrc, metric, unit, fig_no, ymin, ymax, brk)
...
```{r}
fig_no = "E.15.GasNoCCSGen" # w/o CCS
...
fig = four_corners("Electricity Generation from Gas without CCS", ts_map_ID, pd_map_ID, ad_map_ID, drop, histsrc, metric, unit, fig_no, ymin, ymax, brk)
...
#Includes Biomass, Hydro, Geothermal, Oil w/ CCS, and Oil w/o CCS
ts_map_ID = 88
pd_map_ID = 77
ad_map_ID = 11
drop = c("EIAinEMF37.csv")
...
# The percent difference figure here gets a little wonky because of the multiple variables.
...
pdfigure = ggplot() +
  ...
  IRAfigure = ggplot(df[df$scenario == "IRA", ], aes(year, value, color = model)) +
    ...
    labs(title = "IRA",
         ...
         x = element_blank()) +
    theme(axis.ticks = element_line(color = "black"),
           axis.ticks.length = unit(-0.15, "cm"),legend.position = "right",
          plot.title = element_text(hjust = 0.5),
          axis.text.x = element_text(angle = 45, hjust = 1)) +
    geom_point(aes(x = 2021, y = df$value[df$year == 2021][1]), color = "black") +
    geom_point(
...
df$variable_rename = "Other Generation"

abdata = abdata %>%
  mutate(variable_rename = "Other Generation",
         figure_num = fig_no) #%>%
  #rename("value" = "abdif")
...
data_combo = combine_4c_data(df, pbdata, abdata, metric, unit)
write.csv(data_combo, file = paste0("output/final_figures/data/Fig",fig_no,".csv"), row.names = FALSE)
...
data_extract1 = bind_rows(EV_IRA, EV_noIRA) %>% ungroup() %>%
  select(model, scenario, unit, variable_rename, year, value) %>%
  mutate(figure_num = fig_no)
...
data_extract3 = df %>%
  mutate(scenario = "IRA",
         figure_num = fig_no) %>%
  rename("variable_rename" = "name","diff" = "value")
...
EV_pdif_figure = ggplot() +
  ...
  data_combo = combine_4c_data(data_extract1, data_extract2, data_extract3, metric, unit)
  write.csv(data_combo, file = paste0("output/final_figures/data/Fig",fig_no,".csv"), row.names = FALSE)
...
dta = dta %>% ungroup() %>%
  ...
  select(figure_num, model, scenario, unit, variable, label, year, value)
...
```{r}
fig_no = "E.3.gascoal_capacityfactor"
...
```{r}
fig_no = "E.20.elec_share_econwide"
...
NoIRAfigure = ggplot(ts_df[ts_df$scenario == "No IRA",], aes(year, value, color = model)) +
  ...
  scale_y_continuous(limits = c(0, 31), breaks = c(0, 15, 30), labels = scales::comma) +
    scale_linetype_manual(values = c("EPS-EI" = "solid",
                                     "GCAM-CGS" = "solid",
                                     "GCAM-PNNL" = "solid",
                                     "Haiku-RFF" = "solid",
                          ...
                          breaks = c("EPS-EI",
                                     ...
                                     "USREP-ReEDS")) +
  labs(title = "No IRA",
       y = expression(paste("% Electricity of Final Energy")),
       x = element_blank()) + 
  theme(axis.ticks = element_line(color = "black"),
           axis.ticks.length = unit(-0.15, "cm"),
    plot.title = element_text(hjust = 0.5),
  ...
  geom_point(aes(x = 2020, y = ts_df$value[ts_df$year == 2020][1]), color = "black") +
  geom_point(
    data = noIRAmedians,
    aes(x = year, y = median),
    color = "black",
...
difs = ts_df %>%
  group_by(model, year) %>%
  summarize(absdif = (value[scenario == "IRA"] - value[scenario == "No IRA"]), 
          pbdif = (value[scenario == "IRA"] - value[scenario == "No IRA"])/value[scenario=="No IRA"]) %>%
  mutate(pbdif = pbdif * 100)
...
pbdif_figure = ggplot() +
    ...
    labs(title = "",
         x = "",
         y = expression(paste("Percent Difference (%)"))) 
...
medians = absdifs %>%
  group_by(year) %>%
  filter(!year == 2025) %>%
  summarize(median = median(absdif))

absdif_figure = ggplot() +
    geom_line(data = absdifs,
              aes(
                x = year,
                y = absdif,
                group = model,
    ...
    labs(title = "",
         x = "",
         y = expression(paste("Absolute Difference (pp)"))) 

fig = (NoIRAfigure |
         IRAfigure) / (absdif_figure | pbdif_figure) +
  plot_layout(guides = "collect")
fig
...
ts_df = ts_df %>%
  mutate(unit = "Mt C02/yr") %>%
  mutate(figure_num = fig_no) %>%
  select(figure_num, model, scenario, variable, unit, year, value)

pbdifs = pbdifs %>%
  mutate(unit = "% Difference from No IRA", value = pbdif, figure_num = fig_no, scenario = "% Difference", variable = "Economy-Wide") %>%
  select(figure_num, model, scenario, variable, unit, year, value)

absdifs = absdifs %>%
  mutate(unit = "Absolute Difference from No IRA", value = absdif, figure_num = fig_no, scenario = "Absolute Difference", variable = "Economy-Wide") %>%
  select(figure_num, model, scenario, variable, unit, year, value)

data = rbind(ts_df, absdifs, pbdifs)

write.csv(data, file =  paste0("output/final_figures/data/Fig",fig_no,".csv"), row.names = FALSE)
```

### Transportation
```{r}
fig_no = "E.21.elec_share_transportation"
...
NoIRAfigure = ggplot(ts_df[ts_df$scenario == "No IRA",], aes(year, value, color = model)) +
  ...
  theme_emf()+
  scale_x_continuous(breaks = c(2020, 2025, 2030, 2035)) +
  scale_y_continuous(limits = c(0, 16), breaks = c(0, 5, 10, 15), labels = scales::comma) +
    scale_linetype_manual(values = c("EPS-EI" = "solid",
                                     "GCAM-CGS" = "solid",
                                     "GCAM-PNNL" = "solid",
                                     "Haiku-RFF" = "solid",
...
ts_df = ts_df %>%
  mutate(unit = "% Electricity of Final Energy") %>%
  mutate(figure_num = fig_no) %>%
  select(figure_num, model, scenario, variable, unit, year, value)
...
absdifs = absdifs %>%
  mutate(unit = "Absolute Difference from No IRA", value = absdif, figure_num = fig_no, scenario = "% Difference", variable = "Economy-Wide") %>%
  select(figure_num, model, scenario, variable, unit, year, value)
...
```{r}
fig_no = "E.22.elec_share_buildings"
...
NoIRAfigure = ggplot(ts_df[ts_df$scenario == "No IRA",], aes(year, value, color = model)) +
  ...
  theme_emf()+
  scale_x_continuous(breaks = c(2020, 2025, 2030, 2035)) +
  scale_y_continuous(limits = c(0, 66), breaks = c(0, 20, 40, 60), labels = scales::comma) +
    scale_linetype_manual(values = c("EPS-EI" = "solid",
                                     "GCAM-CGS" = "solid",
                                     "GCAM-PNNL" = "solid",
                                     "Haiku-RFF" = "solid",
...
```{r}
fig_no = "E.23.elec_share_industry"
...
NoIRAfigure = ggplot(ts_df[ts_df$scenario == "No IRA",], aes(year, value, color = model)) +
  ...
  theme_emf()+
  scale_x_continuous(breaks = c(2020, 2025, 2030, 2035)) +
  scale_y_continuous(limits = c(10, 20), breaks = c(10, 15, 20), labels = scales::comma) +
    scale_linetype_manual(values = c("EPS-EI" = "solid",
                                     "GCAM-CGS" = "solid",
                                     "GCAM-PNNL" = "solid",
                                     "Haiku-RFF" = "solid",
...
```{r}
fig_no = "E.24.ew_projections_direct"
...
ts_df = spg_clean(
  ...
  select(model, scenario, unit, title_name, year, value) %>%
  mutate(title_name = "Economy-Wide Direct Emissions")
...
NoIRAfigure = ggplot(ts_df[ts_df$scenario == "No IRA",], aes(year, value, color = model)) +
  ...
  theme_emf()+
  scale_x_continuous(breaks = c(2021, 2025, 2030, 2035)) +
  scale_y_continuous(limits = c(2500, 5500), breaks = c(2500, 4000, 5500), labels = scales::comma) +
    scale_linetype_manual(values = c("EPS-EI" = "solid",
                                     "GCAM-CGS" = "solid",
                                     "GCAM-PNNL" = "solid",
                                     "Haiku-RFF" = "solid",
...
absdif_figure = ggplot() +
    ...
    labs(title = "",
         x = "",
         y = expression(paste("Absolute Difference (Emissions (Mt C", O[2], "/yr))"))) 
...
ts_df_data = ts_df %>%
  mutate(variable = "Economy-Wide Direct Emissions", figure_num = fig_no) %>%
  ungroup() %>%
  select(figure_num, model, scenario, variable, unit, year, value)

pbdifs_data = pbdifs %>%
  mutate(unit = "% Difference from No IRA", value = pbdif, figure_num = fig_no, scenario = "% Difference", variable = "Economy-Wide Direct Emissions") %>%
  select(figure_num, model, scenario, variable, unit, year, value)

absdifs_data = absdifs %>%
  mutate(unit = "Absolute Difference from No IRA", value = absdif, figure_num = fig_no, scenario = "Absolute Difference", variable = "Economy-Wide Direct Emissions") %>%
  select(figure_num, model, scenario, variable, unit, year, value)

data = rbind(ts_df_data, absdifs_data, pbdifs_data)
...
```{r}
fig_no = "E.25.transportation_projections_direct"
...
ts_df = spg_clean(
  ...
  filter(year == 2021 | year == 2025 | year == 2030 | year == 2035) %>%
  select(model, scenario, unit, title_name, year, value) %>%
  mutate(title_name = "Transportation Direct Emissions")
...
NoIRAfigure = ggplot(ts_df[ts_df$scenario == "No IRA",], aes(year, value, color = model)) +
  ...
  theme_emf()+
  scale_x_continuous(breaks = c(2021, 2025, 2030, 2035)) +
  scale_y_continuous(limits = c(1000, 2000), breaks = c(1000, 1500, 2000), labels = scales::comma) +
    scale_linetype_manual(values = c("EPS-EI" = "solid",
                                     "GCAM-CGS" = "solid",
                                     "GCAM-PNNL" = "solid",
                                     "Haiku-RFF" = "solid",
...
ts_df_data = ts_df %>%
  mutate(variable = "Transportation Direct Emissions", figure_num = fig_no) %>%
  ungroup() %>%
  select(figure_num, model, scenario, variable, unit, year, value)

pbdifs_data = pbdifs %>%
  mutate(unit = "% Difference from No IRA", value = pbdif, figure_num = fig_no, scenario = "% Difference", variable = "Transportation Direct Emissions") %>%
  select(figure_num, model, scenario, variable, unit, year, value)

absdifs_data = absdifs %>%
  mutate(unit = "Absolute Difference from No IRA", value = absdif, figure_num = fig_no, scenario = "Absolute Difference", variable = "Transportation Direct Emissions") %>%
  select(figure_num, model, scenario, variable, unit, year, value)
...
```{r}
fig_no = "E.26.buildings_projections_direct"
...
ts_df = spg_clean(
  ...
  filter(year == 2021 | year == 2025 | year == 2030 | year == 2035) %>%
  select(model, scenario, unit, title_name, year, value) %>%
  mutate(title_name = "Buildings Direct Emissions")
...
NoIRAfigure = ggplot(ts_df[ts_df$scenario == "No IRA",], aes(year, value, color = model)) +
  ...
  theme_emf()+
  scale_x_continuous(breaks = c(2021, 2025, 2030, 2035)) +
  scale_y_continuous(limits = c(200, 700), breaks = c(200, 450, 700), labels = scales::comma) +
    scale_linetype_manual(values = c("EPS-EI" = "solid",
                                     "GCAM-CGS" = "solid",
                                     "GCAM-PNNL" = "solid",
                                     "Haiku-RFF" = "solid",
...
ts_df_data = ts_df %>%
  mutate(variable = "Buildings Direct Emissions", figure_num = fig_no) %>%
  ungroup() %>%
  select(figure_num, model, scenario, variable, unit, year, value)

pbdifs_data = pbdifs %>%
  mutate(unit = "% Difference from No IRA", value = pbdif, figure_num = fig_no, scenario = "% Difference", variable = "Buildings Direct Emissions") %>%
  select(figure_num, model, scenario, variable, unit, year, value)

absdifs_data = absdifs %>%
  mutate(unit = "Absolute Difference from No IRA", value = absdif, figure_num = fig_no, scenario = "Absolute Difference", variable = "Buildings Direct Emissions") %>%
  select(figure_num, model, scenario, variable, unit, year, value)
...
```{r}
fig_no = "E.27.industry_projections_direct"
...
ts_df = spg_clean(
  ...
  filter(year == 2021 | year == 2025 | year == 2030 | year == 2035) %>%
  select(model, scenario, unit, title_name, year, value) %>%
  mutate(title_name = "Industry Direct Emissions")
...
NoIRAfigure = ggplot(ts_df[ts_df$scenario == "No IRA",], aes(year, value, color = model)) +
  ...
  theme_emf()+
  scale_x_continuous(breaks = c(2021, 2025, 2030, 2035)) +
  scale_y_continuous(limits = c(600, 1250), breaks = c(600, 800, 1000, 1200), labels = scales::comma) +
    scale_linetype_manual(values = c("EPS-EI" = "solid",
                                     "GCAM-CGS" = "solid",
                                     "GCAM-PNNL" = "solid",
                                     "Haiku-RFF" = "solid",
...
ts_df_data = ts_df %>%
  mutate(variable = "Industry Direct Emissions", figure_num = fig_no) %>%
  ungroup() %>%
  select(figure_num, model, scenario, variable, unit, year, value)

pbdifs_data = pbdifs %>%
  mutate(unit = "% Difference from No IRA", value = pbdif, figure_num = fig_no, scenario = "% Difference", variable = "Industry Direct Emissions") %>%
  select(figure_num, model, scenario, variable, unit, year, value)

absdifs_data = absdifs %>%
  mutate(unit = "Absolute Difference from No IRA", value = absdif, figure_num = fig_no, scenario = "Absolute Difference", variable = "Industry Direct Emissions") %>%
  select(figure_num, model, scenario, variable, unit, year, value)
...
```{r}
fig_no = "E.28.ew_projections_indirect"
...
ts_df = merge(Econ_data, Elec_data, by = c("model", "scenario", "year")) %>%
...
NoIRAfigure = ggplot(ts_df[ts_df$scenario == "No IRA",], aes(year, value, color = model)) +
  ...
  theme_emf()+
  scale_x_continuous(breaks = c(2021, 2025, 2030, 2035)) +
  scale_y_continuous(limits = c(2000, 4000), breaks = c(2000, 3000, 4000), labels = scales::comma) +
    scale_linetype_manual(values = c("EPS-EI" = "solid",
                                     "GCAM-CGS" = "solid",
                                     "GCAM-PNNL" = "solid",
                                     "Haiku-RFF" = "solid",
...
ts_df_data = ts_df %>%
  mutate(variable = "Economy-Wide Indirect Emissions", figure_num = fig_no, unit = "Mt CO2/yr") %>%
  ungroup() %>%
  select(figure_num, model, scenario, variable, unit, year, value)

pbdifs_data = pbdifs %>%
  mutate(unit = "% Difference from No IRA", value = pbdif, figure_num = fig_no, scenario = "% Difference", variable = "Economy-Wide Indirect Emissions") %>%
  select(figure_num, model, scenario, variable, unit, year, value)

absdifs_data = absdifs %>%
  mutate(unit = "Absolute Difference from No IRA", value = absdif, figure_num = fig_no, scenario = "Absolute Difference", variable = "Economy-Wide Indirect Emissions") %>%
  select(figure_num, model, scenario, variable, unit, year, value)
...
```{r}
fig_no = "E.29.transportation_projections_indirect"
...
NoIRAfigure = ggplot(ts_df[ts_df$scenario == "No IRA",], aes(year, value, color = model)) +
  ...
  theme_emf()+
  scale_x_continuous(breaks = c(2021, 2025, 2030, 2035)) +
  scale_y_continuous(limits = c(0, 125), breaks = c(0, 30, 60, 90, 120), labels = scales::comma) +
    scale_linetype_manual(values = c("EPS-EI" = "solid",
                                     "GCAM-CGS" = "solid",
                                     "GCAM-PNNL" = "solid",
                                     "Haiku-RFF" = "solid",
...
ts_df_data = ts_df %>%
  mutate(variable = "Transportation Indirect Emissions", figure_num = fig_no) %>%
  ungroup() %>%
  select(figure_num, model, scenario, variable, unit, year, value)

pbdifs_data = pbdifs %>%
  mutate(unit = "% Difference from No IRA", value = pbdif, figure_num = fig_no, scenario = "% Difference", variable = "Transportation Indirect Emissions") %>%
  select(figure_num, model, scenario, variable, unit, year, value)

absdifs_data = absdifs %>%
  mutate(unit = "Absolute Difference from No IRA", value = absdif, figure_num = fig_no, scenario = "Absolute Difference", variable = "Transportation Indirect Emissions") %>%
  select(figure_num, model, scenario, variable, unit, year, value)
...
```{r}
fig_no = "E.30.buildings_projections_indirect"
...
NoIRAfigure = ggplot(ts_df[ts_df$scenario == "No IRA",], aes(year, value, color = model)) +
  ...
  theme_emf()+
  scale_x_continuous(breaks = c(2021, 2025, 2030, 2035)) +
  scale_y_continuous(limits = c(200, 1200), breaks = c(200, 700, 1200), labels = scales::comma) +
    scale_linetype_manual(values = c("EPS-EI" = "solid",
                                     "GCAM-CGS" = "solid",
                                     "GCAM-PNNL" = "solid",
                                     "Haiku-RFF" = "solid",
...
ts_df_data = ts_df %>%
  mutate(variable = "Buildings Indirect Emissions", figure_num = fig_no) %>%
  ungroup() %>%
  select(figure_num, model, scenario, variable, unit, year, value)

pbdifs_data = pbdifs %>%
  mutate(unit = "% Difference from No IRA", value = pbdif, figure_num = fig_no, scenario = "% Difference", variable = "Buildings Indirect Emissions") %>%
  select(figure_num, model, scenario, variable, unit, year, value)

absdifs_data = absdifs %>%
  mutate(unit = "Absolute Difference from No IRA", value = absdif, figure_num = fig_no, scenario = "Absolute Difference", variable = "Buildings Indirect Emissions") %>%
  select(figure_num, model, scenario, variable, unit, year, value)
...
```{r}
fig_no = "E.31.industry_projections_indirect"
...
NoIRAfigure = ggplot(ts_df[ts_df$scenario == "No IRA",], aes(year, value, color = model)) +
  ...
  theme_emf()+
  scale_x_continuous(breaks = c(2021, 2025, 2030, 2035)) +
  scale_y_continuous(limits = c(0, 500), breaks = c(0, 250, 500), labels = scales::comma) +
    scale_linetype_manual(values = c("EPS-EI" = "solid",
                                     "GCAM-CGS" = "solid",
                                     "GCAM-PNNL" = "solid",
                                     "Haiku-RFF" = "solid",
...
ts_df_data = ts_df %>%
  mutate(variable = "Industry Indirect Emissions", figure_num = fig_no) %>%
  ungroup() %>%
  select(figure_num, model, scenario, variable, unit, year, value)

pbdifs_data = pbdifs %>%
  mutate(unit = "% Difference from No IRA", value = pbdif, figure_num = fig_no, scenario = "% Difference", variable = "Industry Indirect Emissions") %>%
  select(figure_num, model, scenario, variable, unit, year, value)

absdifs_data = absdifs %>%
  mutate(unit = "Absolute Difference from No IRA", value = absdif, figure_num = fig_no, scenario = "Absolute Difference", variable = "Industry Indirect Emissions") %>%
  select(figure_num, model, scenario, variable, unit, year, value)
...
```{r}
fig_no = "F.1.1.electricity_share_final_energy"
...
EconSpg = spg3(df = Econ_ts, 
               title = "Economy-Wide", 
               yname = expression("% Electricity of Final Energy"),
               gd = "none", 
               ymin = 0,
               ymax = 70, 
...
EconSpg$data = EconSpg$data %>%
  ungroup() %>%
  mutate(figure_num = fig_no, variable = "Electricity Share") %>%
  rename("label" = "variable_rename") %>%
  select(-alpha)

EconDot = dotted(Econ_ts, EconSpg$figure, "median", 0, 70, config, figmap_leep_timeseries, ddge = -0.2)
...
TranDot = dotted(Tran_ts, TranSpg$figure, "median", 0, 70, config, figmap_leep_timeseries, ddge = -0.2)
...
BuilSpg$data = BuilSpg$data %>%
  ungroup() %>%
  mutate(figure_num = fig_no, variable = "Electricity Share") %>%
  rename("label" = "variable_rename") %>%
  select(-alpha)

BuilDot = dotted(Buil_ts, BuilSpg$figure, "median", 0, 70, config, figmap_leep_timeseries, ddge = -0.2)
...
IndSpg$data = IndSpg$data %>%
  ungroup() %>%
  mutate(figure_num = fig_no, variable = "Electricity Share") %>%
  rename("label" = "variable_rename") %>%
  select(-alpha)

data_combo = bind_rows(EconSpg$data, TranSpg$data, BuilSpg$data, IndSpg$data)
savedata(data_combo, fig_no)

IndDot = dotted(Ind_ts, IndSpg$figure, "median", 0, 70, config, figmap_leep_timeseries, ddge = -0.2)

Full = (EconDot | TranDot) / (BuilDot | IndDot)
Full

savefig(Full, fig_no, wd = 8, ht = 8)
```
...
data_combo = all
savedata(data_combo, paste0(fig_no,"_summary"))

#saveWorkbook(per_share_wb, file = "output/final_figures/data/TableF.1.1.share.xlsx", overwrite = TRUE)
```

```{R}
# display table for appendix F1
fig_no = "F.1.1.Electricity Share"
...
table1 = tab_dta %>% gt() %>%
    ...
    ) %>% fmt_percent(decimals = 0, use_seps = TRUE, columns = 3:8)
...
#saveWorkbook(per_share_wb, file = "output/final_figures/data/TableF.1.2.difference.xlsx", overwrite = TRUE)
```

```{R}
# display table for appendix G2
fig_no = "F.1.2 Electricity Difference"
tab_dta = all_per %>% 
  data.frame() %>%
...
## F.2 Direct and indirect emissions breakouts

### EconWide Direct/Indirect

```{r}
fig_no = "F.2.1.ew_projections_direct_indirect"
...
savefig(fig, fig_no, wd=9,ht=4)

EconSpg$data = EconSpg$data %>%
  ungroup() %>%
  select(-alpha) %>%
  rename("label" = "variable_rename") %>%
  mutate(variable = "Emissions|CO2|Economy-Wide",
         figure_num = fig_no,
         unit = "Mt CO2/yr")
...
data_combo = bind_rows(EconSpg$data, ElecSpg$data)
savedata(data_combo, fig_no)
```
...
```{r}
fig_no = "F.2.2.trn_projections_direct_indirect"
...
DirSpg$data = DirSpg$data %>%
  ungroup() %>%
  mutate(figure_num = fig_no) %>%
  rename("label" = "variable_rename") %>%
  select(figure_num, model, scenario, variable, unit, label, year, value)

IndDirSpg$data = IndDirSpg$data %>%
  ungroup() %>%
  mutate(figure_num = fig_no) %>%
  rename("label" = "variable_rename") %>%
  select(figure_num, model, scenario, variable, unit, label, year, value)

data_combo = bind_rows(DirSpg$data, IndDirSpg$data)
savedata(data_combo, fig_no)
...
```{r}
fig_no = "F.2.3.bld_projections_direct_indirect"
...
```{r}
fig_no = "F.2.4.ind_projections_direct_indirect"
...
dots_Total = ggplot(data = data_total, aes(x = axe, y = value,  color = scenario)) +
  geom_point(aes(alpha = alpha, shape = scenario), size = 1.5, position = position_dodge(width = 0.5)) +
...
savefig(final, fig_no, wd = 8, ht = 8)

data_combo = bind_rows(data_total,data_Low,data_High,data_Nuclear,data_Solar,data_Wind,data_CCS,data_Coal,data_Gas,data_Other) %>%
  ungroup() %>%
  select(-c(alpha, axe)) %>%
  rename("label" = "variable_rename") %>%
...
```{r}
title = "F.3.1.Total Electricity Generation"
...
```{r}
fig_no = "F.3.2.genbytype_differences"
...
dots_Wind = ggplot(data = data_Wind, aes(x = axe, y = abs_dif)) +
  ...
  geom_segment(data = df_stat_30, aes(x = 1.94, xend = 2.06, y = median, yend = median, color = scenario), size = 0.5) +
    ...
    scale_x_continuous(breaks = c(1,2), labels = c("2030", "2035")) +
    theme_emf() +
    labs(y = "Difference from No IRA (TWh)") +
    scale_subpalette(subpalettes, "Emissions|CO2|Energy|Demand|Industry") +
    theme(
          axis.title.x = element_blank(),
          axis.title.y = element_text(size = 7),
...
data_combo = bind_rows(data_total,data_Low,data_High,data_Nuclear,data_Solar,data_Wind,data_CCS,data_Coal,data_Gas,data_Other) %>%
  ...
  mutate(variable = "Secondary Energy|Electricity",
         unit = "TWh",
         figure_num = fig_no) %>%
  select(figure_num, model, scenario, variable, unit, label,year, `No IRA`, IRA, Abs_Diff)
...
```{r}
fig_no = "F.3.2"
title = "F.3.2.Low Emitting Electricity Generation"

data_table = data_Low %>%
  group_by(year) %>%
  summarize(max_IRA = max(IRA), 
            min_IRA = min(IRA), 
            med_IRA = median(IRA), 
            max_diff = max(abs_dif), 
            min_diff = min(abs_dif), 
            med_diff = median(abs_dif), 
            max_noIRA = max(`No IRA`), 
            min_noIRA = min(`No IRA`), 
            med_noIRA = median(`No IRA`)) %>%
  pivot_longer(cols = !year) %>%
  mutate(stat = case_when(
    grepl("min", name) ~ "Min", 
    grepl("max", name) ~ "Max", 
    grepl("med", name) ~ "Median"
  ))%>%
  mutate(name = case_when(
    grepl("noIRA", name) ~ "No IRA", 
    grepl("IRA", name) ~ "IRA", 
    T ~ "Difference"
  ))%>%
  pivot_wider(names_from = stat, values_from = value) %>%
  select(Year = year, Scenario = name, Minimum = Min, Maximum = Max, Median)
  
  write.csv(data_table, paste("./output/final_figures/stats_tables/", fig_no, "SummaryTable ",title,".csv",sep=""))
```
...
```{r}
fig_no = "I.2"
title = "I.Natural Gas Electricity Generation"
...
## F.4 Sensitivity Range Summary

### All sensitivities

```{r}
fig_no = "F.4.1.summary_ranges_sensitivities"

clean_data1 = clean_data1 = filter_sensitivities(clean_data)
...
indicators_all_clean = indicators_all %>% 
  select(model,scenario,unit,region,category,year,value,value_2005,pct_diff) %>% 
...
EconTab = html(Econ_data, "Economy-Wide CO2")
...
Econ_data = clean_supplemental_data(Econ_data, fig_no)
...
#savedata(Econ_data, paste(fig_no, "EconWide"))
...
#savedata(summary_all, paste(fig_no,"summary",sep="_"))
```

### Core ranges to calculate difference
...
savedata(joint, paste(fig_no,"changes",sep="_"))
```

# Appendix G: Buildings sector modeling

## Emissions reudctions by end use type from scout modeling

```{r}
fig_no = "G.1.1.scout_results"
...
fin = ggplot(data = data_sort[data_sort$variable_rename != "Reference",] , aes(x=year, y=value, fill = variable_rename)) +
  ...
  labs(title = "",
         x = "",
         y = expression(paste("Building C", O[2], "Emissions (Mt C", O[2], "/yr)")),
         color = "") +
    scale_y_continuous(limits = c(0, 1500), labels = scales::comma, expand = c(0,0)) +
    theme_emf() +
    scale_x_continuous(breaks = c(2025, 2030, 2035)) + 
    theme(axis.text.x = element_text(angle = 45, hjust=1), axis.ticks = element_line(color="black"), axis.ticks.length = unit(-0.15, "cm")) +
  ...
  scale_fill_manual(name = "", 
    ...
    values = c("#488C49", '#f15a22', "#af499b",  '#0083ca', "#DEDEDE", "#767684"),
    limits = c("HVAC", "WaterHeating", "Other", "Power", "Total") 
  ) + 
  geom_line(data = data_sort[data_sort$variable_rename == "Reference",],  aes(x=year, y=value, color = variable_rename),
            linetype="dashed", size = 1) +
  scale_color_manual(name = "", 
    labels = c("HVAC/Env", "Water Heating", "Other" , "Power Supply Decarbonizaiton", "Total Building Emissions", "Reference"),
    breaks = c("HVAC", "WaterHeating", "Other","ower", "Total", "Reference"), 
    values = c("white", "white", "white", "white", "white", "black"),
    limits = c("Reference") 
  )
fin

data_sort = data_sort %>% mutate(figure_num = fig_no, model = "Scout-LEEP", 
                                 variable = "Emissions|CO2|Energy|Demand|Buildings", unit = "Mt CO2/yr") %>%
  rename("label" = "variable_rename")

saveall(fin, data_sort, fig_no, ht = 9, wd = 7.5)
```

# NOT USED: Appendinx L Relationships between number of provisions modeled and emissions reductions

```{r}
dta = read.csv("./data-extra/emis_reg_data.csv")
#View(dta)

reg = lm(elec_pctdiff_2035 ~ elec_prov, data = dta)
print(summary(reg))

p = dta %>% ggplot() +
  geom_point(aes(x = elec_prov, y = elec_pctdiff_2035), color = "darkblue", size = 4, shape = 16) +
  geom_point(aes(x = elec_prov, y = elec_pctdiff_2030), color = "lightblue", size = 4, shape = 16) +
  geom_text_repel(aes(x = elec_prov, y = elec_pctdiff_2035, label = abbr), 
                  color = "darkblue", size = 4, vjust = -0.2, hjust = 1.5, seed = 42) + 
  geom_text_repel(aes(x = elec_prov, y = elec_pctdiff_2030, label = abbr), 
                  color = "lightblue", size = 4, vjust = -0.2, hjust = 1.5, seed = 42) + 
  annotate("text", x = 3.7, y = -0.025, label = "2030", color = "lightblue", alpha = 1, fontface = "bold") +
  annotate("text", x = 3.7, y = -0.075, label = "2035", color = "darkblue", alpha = 1, fontface = "bold") +
  ...
  scale_x_continuous(limits = c(3,10), name = "Number of Electric Sector Provisions Modeled") +
  scale_y_continuous(limits = c(-1,0), name = expression("Change in Electric Sector Emissions (%, relative to No IRA)")) +
  theme_emf()
p
```
