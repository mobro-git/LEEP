...
generation_data = clean_data %>%
  filter(year %in% c(2030, 2035),
         !model %in% exclude_models,
         variable %in% c("Secondary Energy|Electricity|Coal", "Secondary Energy|Electricity|Gas"),
...
capacity_data = clean_data %>%
  filter(year %in% c(2030, 2035),
         !model %in% exclude_models,
         variable %in% c("Capacity|Electricity|Coal","Capacity|Electricity|Gas"),
         #variable %in% c("Secondary Energy|Electricity|Coal", "Secondary Energy|Electricity|Gas"),
         scenario %in% c("IRA","No IRA","Core")) %>%
...
cf_data = generation_data_temp %>% inner_join(capacity_data_temp, by = c("Model", "Generation.Fuel")) %>%
...
cf_table = gt_elec_table(cf_data, "Electricity Generation Capacity Factors (%)", percent = TRUE)
cf_table
