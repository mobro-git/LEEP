---
title: "Transportation Study Group Presentation Figures"
author: "Ruying Gao"
date: "`r Sys.Date()`"
output: 
  html_document:
...
tar_load(tsg_sb_presentation_figmap)
stopifnot(exists("tsg_sb_presentation_figmap"))

tar_load(tsg_db_presentation_figmap)
stopifnot(exists("tsg_db_presentation_figmap"))

tar_load(tsg_ts_presentation_figmap)
stopifnot(exists("tsg_ts_presentation_figmap"))
```

```{r file path setup}
overall_path = "./output/round1/tsg_presentation/"
subfolders = c("", "stacked_bar", "diff_bar", "time_series")
create_folders(sapply(subfolders, function(x) {paste(overall_path, x, sep = "")}))
```

```{r tsg plotting functions, echo=FALSE}
tsg_stacked_bar_grid_fn <- function(df, figure, selected_region, sub_palettes) {
...
tsg_stacked_bar_wrap_fn <- function(df, figure, selected_region, sub_palettes) {
...
tsg_time_series_grid_fn <- function(df, figure, selected_region, sub_palettes) {
...
tsg_time_series_wrap_fn <- function(df, figure, selected_region, sub_palettes) {
...
plot_list <- tsg_sb_presentation_figmap
```
...
pdf(file=paste(overall_path, "tsg_stackbar_presentation.pdf",sep=""), width = 14, height = 6)
...
for (figure in unique(df$title_name)) {
    ...
    for (selected_region in unique(dat$region)) {
      ...
      if (unique(dat_filtered$type) == "grid") {
          print(tsg_stacked_bar_grid_fn(dat_filtered, figure, selected_region, sub_palettes))
      } else if (unique(dat_filtered$type) == "wrap") {
          print(tsg_stacked_bar_wrap_fn(dat_filtered, figure, selected_region, sub_palettes))
      } else {
...
plot_list <- tsg_db_presentation_figmap
```
...
pdf(file=paste(overall_path, "tsg_diffbar_presentation.pdf",sep=""), width = 14, height = 6)
print(paste("There are a total of ", length(unique(df$title_name)), " figures", sep = ""))
i = 0
for (figure in unique(df$title_name)) {
    print(figure)
    ...
    for (selected_region in unique(dat$region)) {
      ...
      if (check_dat_before_plotting(dat_filtered)) {
        if (unique(dat_filtered$type) == "grid") {
            bar_plot = tsg_stacked_bar_grid_fn(dat_filtered, figure, selected_region, sub_palettes)
          
        } else if (unique(dat_filtered$type) == "wrap") {
            bar_plot = tsg_stacked_bar_wrap_fn(dat_filtered, figure, selected_region, sub_palettes)
        } else {
          print("There is no corresponding graphic function available yet.")
        }
...
# Part 3. Time Series
```{r merge - time series}
plot_list <- tsg_ts_presentation_figmap
df <- preliminary_data_processing_for_plotting(emf_data_long, plot_list)
```
...
pdf(file=paste(overall_path, "tsg_time_series_presentation.pdf",sep=""), width = 14, height = 6)
print(paste("There are a total of ", length(unique(df$title_name)), " figures", sep = ""))
i = 0
for (figure in unique(df$title_name)) {
    ...
    for (selected_region in unique(dat$region)) {
      ...
      if (check_dat_before_plotting(dat_filtered)) {
      if (unique(dat_filtered$type) == "grid") {
          print(tsg_time_series_grid_fn(dat_filtered, figure, selected_region, sub_palettes))
      } else if (unique(dat_filtered$type) == "wrap") {
         print(tsg_time_series_wrap_fn(dat_filtered, figure, selected_region, sub_palettes))
      } else {
        print("There is no corresponding graphic function available yet.")
      }
    }
