...
```{r file path setup}
overall_path = "./output/round1/overview_presentation/"
subfolders = c("", "stacked_bar", "diff_bar", "time_series")
create_folders(sapply(subfolders, function(x) {paste(overall_path, x, sep = "")}))
```

```{r overview plotting functions, echo=FALSE}
overview_stacked_bar_grid_fn <- function(df, figure, selected_region, sub_palettes) {
...
overview_stacked_bar_wrap_fn <- function(df, figure, selected_region, sub_palettes) {
...
overview_time_series_grid_fn <- function(df, figure, selected_region, sub_palettes) {
...
overview_time_series_wrap_fn <- function(df, figure, selected_region, sub_palettes) {
...
pdf(file=paste(overall_path, "overview_stackbar_presentation.pdf",sep=""), width = 14, height = 6)
...
for (figure in unique(df$title_name)) {
    ...
    for (selected_region in unique(dat$region)) {
      ...
      if (unique(dat_filtered$type) == "grid") {
          print(overview_stacked_bar_grid_fn(dat_filtered, figure, selected_region, sub_palettes))
      } else if (unique(dat_filtered$type) == "wrap") {
          print(overview_stacked_bar_wrap_fn(dat_filtered, figure, selected_region, sub_palettes))
      } else {
...
pdf(file=paste(overall_path, "overview_diffbar_presentation.pdf",sep=""), width = 14, height = 6)
print(paste("There are a total of ", length(unique(df$title_name)), " figures", sep = ""))
i = 0
for (figure in unique(df$title_name)) {
    print(figure)
    ...
    for (selected_region in unique(dat$region)) {
      ...
      if (check_dat_before_plotting(dat_filtered)) {
        if (unique(dat_filtered$type) == "grid") {
            bar_plot = overview_stacked_bar_grid_fn(dat_filtered, figure, selected_region, sub_palettes)
          
        } else if (unique(dat_filtered$type) == "wrap") {
            bar_plot = overview_stacked_bar_wrap_fn(dat_filtered, figure, selected_region, sub_palettes)
        } else {
          print("There is no corresponding graphic function available yet.")
        }
...
pdf(file=paste(overall_path, "overview_time_series_presentation.pdf",sep=""), width = 14, height = 6)
print(paste("There are a total of ", length(unique(df$title_name)), " figures", sep = ""))
i = 0
for (figure in unique(df$title_name)) {
    ...
    for (selected_region in unique(dat$region)) {
      ...
      if (check_dat_before_plotting(dat_filtered)) {
      if (unique(dat_filtered$type) == "grid") {
          print(overview_time_series_grid_fn(dat_filtered, figure, selected_region, sub_palettes))
      } else if (unique(dat_filtered$type) == "wrap") {
         print(overview_time_series_wrap_fn(dat_filtered, figure, selected_region, sub_palettes))
      } else {
        print("There is no corresponding graphic function available yet.")
      }
    }
