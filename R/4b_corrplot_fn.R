...
corrplot_fn <- function(presentation_title, presentation_plot_type, config, emf_data_long) {
  ...
  df <- emf_data_long %>%
    ...
    filter(variable %in% c('Secondary Energy|Electricity|Solar', 'Secondary Energy|Electricity|Wind', 'Final Energy|Transportation|Electricity', 'Carbon Sequestration|Direct Air Capture|Net', 'Secondary Energy|Hydrogen')) %>%
    select(-`unit`) %>%
  ...
  jpeg(file = paste(overall_path, "corrplot_pmetrics_old.jpeg", sep = ""), units="in", width=5, height=5, res=1200)
  # chart.Correlation(M)
  dev.off()
...
#corrplot_fn("elc", "corrplot", config, emf_data_long)
